...
class DataManager:
    ...
    def parseData(self, split, bins=1, keep_bins=False):
        """
        Purpose: created three files (or two files and a folder with files if keep_bins == True) that
        are .csv that states what data (based on unique ID) belong to each cache

        :param split: states what percent of data to place in train, val, and ul cache
        :param bins: states how many ul cache bins to make
        :param keep_bins: bool states if to role over previous ul cache to current
        :return: None
        """
    ...
    def loadCaches(self):
        """
        Purpose: reload cache (train, val, and ul_cache)
        :return: none
        """
